...
SELECT COUNT(*) AS total_num FROM orders WHERE shipto_state = 'CA';
...
SELECT id FROM customers WHERE email LIKE 'pclark74@gmail.com';
...
SELECT id, status, order_total FROM orders WHERE customer_id = ALL(SELECT id FROM customers WHERE email LIKE 'pclark74@gmail.com');
...
SELECT orders.id, orders.status, orders.order_total FROM orders JOIN customers ON (orders.customer_id = customers.id) WHERE customers.email LIKE 'pclark74@gmail.com';
